...
OphcrackGUI::OphcrackGUI(ophcrack_t *crack) : QMainWindow() {
  ...
  tableView->setItemDelegateForColumn(3, progDelegate);
  ...
  localSam   = loadMenu->addAction("Local SAM with pwdump6");
  remoteSam  = loadMenu->addAction("Remote SAM");
#endif
  ...
  connect(localSam, SIGNAL(triggered()), this, SLOT(loadLocalSamPwdump()));
  connect(remoteSam, SIGNAL(triggered()), this, SLOT(loadRemoteSam()));
#endif
...
void OphcrackGUI::loadLocalSamSamdump(void) {  
  ...
  if (npwds == 0) 
    ...
    buff = QString("Error in samdump2: get_sam function\n\nMake sure ophcrack is running with administrator rights to perform this action. On Windows Vista/7, right-click on ophcrack icon and select run as administrator.");
...
//---------------------------------------------------------------------------
void OphcrackGUI::loadLocalSamPwdump(void) {  
  // Retrieve the hashes from the local SAM with pwdump6.
  
  list_t *hashes = list_alloc();
  list_nd_t *nd;

  int maxhid = crack->maxhid;
  int ret = exec_local_pwdump6(hashes, maxhid);

  // If some hashes have been found, then we add them to the current
  // list of hashes.
  ...
  // Otherwise, we notify the user that a problem occured.

  else {
    QMessageBox msgBox(QMessageBox::Warning,
		       "Warning",
		       QString("Cannot dump local SAM with pwdump6."),
		       QMessageBox::Ok);
    msgBox.exec();
  }
...
//---------------------------------------------------------------------------
void OphcrackGUI::loadRemoteSam(void) {
  if (samDialog == 0)
    samDialog = new SamDialog(this);
  
  if (samDialog->exec() == QDialog::Accepted) {
    char *host  = strdup(samDialog->getHost());
    char *share = strdup(samDialog->getShare());
    char *user  = strdup(samDialog->getUser());

    // If the host and share are not empty, then we can proceed.

    if (host[0] != 0 || share[0] != 0) {
      list_t *hashes = list_alloc();
      list_nd_t *nd;
      
      int maxhid = crack->maxhid;
      int ret = exec_remote_pwdump6(hashes, host, share, user, maxhid);
	...
	QMessageBox msgBox(QMessageBox::Warning,
			   "Warning",
			   QString("Cannot dump remote SAM with pwdump6."),
			   QMessageBox::Ok);
	msgBox.exec();
      }
    ...
    // Warn the user that the host and/or the share are empty.

    else {
      QMessageBox msgBox(QMessageBox::Warning,
			 "Warning",
			 QString("You must specify a host and a share."),
			 QMessageBox::Ok);
      msgBox.exec();
    }
...
void OphcrackGUI::updateStatus(void) {
  ...
  double psize_curr  = fsm->psize_curr;
  double psize_total = fsm->psize_total;
  int psize_ratio = 0;

  if (psize_total > 0)
    psize_ratio = (int)ceil(100*psize_curr / psize_total);
  ...
  if (!arg->bforce)
    ...
    double bforce_curr  = fsm->bforce_curr;
    double bforce_total = fsm->bforce_total;
    int bforce_ratio = 0;

    if (bforce_total > 0)
      bforce_ratio = (int)ceil(100*bforce_curr / bforce_total);
